...
public class CLTModel extends LTModel {
    ...
    // seed panic values are used in the diffusion method to udpate the agent panic
    // value
    ...
    public void initialise() {
        ...
        // set initial panic agents based on strategy
    ...
    public void selectContentBasedSeed(String type, int seedSize) {
        ...
        try {
            List<Integer> repIdLsit = getShuffledIDList(); // get shuffled agent id list each time during a seeding
                                                           // strategy
            ...
            for (int id : repIdLsit) {
                ...
                double actProb = getContentBasedProbability(id, agent.getX(), type); // get activation probability based
                                                                                     // on distance and contenttype
    ...
    public double getContentBasedProbability(int id, double easting, String contentType) {
        ...
        try {
            // calcualtes the distance based on the firefront/farend, and then get the
            // proability for the calculated distance
    ...
    public List<Integer> getShuffledIDList() { // tested with small agent sample -5
        ...
        // shuffle the id list, provide random object for deterministic results for
        // testing
    ...
    // if index 0 is not added, then there might be a arrayIndexOutOfBound issue in
    // nei weights arraylists
    public void runCompetitiveDiffusion() {
        ...
        try {
            ...
            for (int id : getAgentMap().keySet()) { // calculate the total content values from seed and neighbour
                                                    // weights.
                ...
                // activation conditions are checked in updateAgent function, therefore if
                // values are less than act thresholds
    ...
    public void updateAgentStateValueAndCounters(int id, String type, Double newVal) {
        ...
        // update content level if greater than previous step - regarldess of activation
        // state
        // If a wait agent becomes panic, then the wait influence can be less than the
        // previuos wait value.
        ...
        /* else */ if (newVal < agent.getContentlevel(type)) {
            logger.error("agent {} , content level {} is less than previous {}", id, newVal,
                    agent.getContentlevel(type));
        }
        ...
        if (agent.getState().equals(DataTypes.LOW) && type.equals(DataTypes.PANIC)
                && newVal >= getAgentContentTreshold(id, type)) { // 2. re-activation (panicked)

            // panic value doesn't matter, even if it is less than the current wait value
            // update state
            // if just modellnig competitive diffusion, then a panic value below threshold
            // can come, then the state should not be changed to H
            // in the fire progression experiment, you should only receive panic values
            // higher than panic threshold.
    ...
    public void printAgentContentValues() {
        ...
        for (SocialAgent agent : getAgentMap().values()) {
            logger.debug("id: {} adopted content:{} waitVal: {} panicVal {}",
                    agent.getID(), agent.getActivatedContentType(), agent.getContentlevel(DataTypes.WAIT),
                    agent.getContentlevel(DataTypes.PANIC));
        }
    }
