...
public class SocialNetworkManager {
	...
	// used in running lhs batch runs, to only set configs other than log, which
	// will be created using a separate method.
	...
	// used when logfile needs to shifted to different directories when executing
	// lhs batch runs
	 ...
	 * Used by SN and SN-BDI and SN-BDI-ABM componants, so includes general
	 * functions,
	 * any specific functions (e.g. publish data to dataserver, write diffusion
	 * data)
	 * should be called in the application side.
	 *
	 * This method includes the constraint of stepsize check before the diffusion
	 * model steps. diffuse() method does not have any restrictions
	 */
	...
	public void createAgentMapWithSpecifiedId(int startId, int numAgents) {
		...
		for (int i = startId; i <= numAgents; i++) {
			...
			setCords(Integer.toString(startId), Global.getRandom().nextInt(maxEasting),
					Global.getRandom().nextInt(maxNorthing));
		}
	}
